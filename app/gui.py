...
class App:
    ...
    def __init__(self, root: tk.Tk):
        self.root = root
        self._files: List[str] = []          # 변환 대기 파일 목록
    ...
    def _build_file_panel(self, parent):
        ...
        ttk.Button(folder_row, text="폴더 선택…", command=self._browse_folder).grid(row=0, column=2, padx=(4, 0))
        ...
        ttk.Button(btn_row, text="+ 파일 추가", command=self._add_files).pack(side="left", padx=(0, 4))
        ttk.Button(btn_row, text="선택 제거", command=self._remove_selected).pack(side="left", padx=4)
        ttk.Button(btn_row, text="전체 지우기", command=self._clear_files).pack(side="left", padx=4)
    ...
    def _on_drop(self, event):
        ...
        raw = event.data
        # tkinterdnd2 는 경로를 {} 로 감싸거나 공백으로 구분해 전달
        ...
        for p in paths:
            ...
            if os.path.isdir(p):
                ...
                if _P(p).suffix.lower() in {".xls", ".xlsx", ".xlsm", ".xlsb"}:
    ...
    def _handle_finished(self, progress: ConversionProgress):
        ...
        summary = f"완료: 성공 {ok}개 / 실패 {fail}개"
        ...
        if fail > 0:
            ...
            messagebox.showinfo("변환 완료", f"모든 파일({ok}개)이 성공적으로 변환되었습니다.")
